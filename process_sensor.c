...
void process_sensors_values(void)
{
	...
	//Les capteurs arrières ne sont pas utilisés//
    ...
    while(1){
    	...
    	if(reference_to_update)
    	{
    		if (sensors_values[LEFT_SENS]==WALL_DETECTED)
    			reference_distance=data_sensors[FRONT_LEFT_45DEG + buffer_state*PROXIMITY_NB_CHANNELS];
    		else
    			reference_distance=data_sensors[FRONT_RIGHT_45DEG + buffer_state*PROXIMITY_NB_CHANNELS];
    		reference_to_update = false;
    	}
		...
		if(maze_mapping_mode_is_selected())
		{
			...
			else
			{
				if (sensors_values[LEFT_SENS]==WALL_DETECTED)
					...
					do_follow_wall_regulation(FRONT_RIGHT_45DEG);
			}
